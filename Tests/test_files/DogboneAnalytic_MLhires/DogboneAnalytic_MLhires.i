...
# HIGH-RESOLUTION BATHYMETRY -- transparency check with a live refined level
# DogboneAnalytic's default (traditional) bathymetry is h = 10 via setVal, so as in
# Channel_Test_hires the average-down must be exact and the DogboneAnalytic_MLvel baseline
# applies unchanged. What this adds is finest_level == 1 with regridding, which covers the
# branches Channel_Test_hires misses: set_bathymetry at lev == hires_grid_level, and the
# hires branches of MakeNewLevelFromCoarse and RemakeLevel.
