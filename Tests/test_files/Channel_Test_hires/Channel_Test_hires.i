...
# components, so the mean over any 3x3 fine block is exactly 50 and the hires path must
# reproduce the plain Channel_Test baseline. This exercises the allocation,
# average_down_with_grow_cells, and the ParallelCopy/FillPatch widths in
# set_bathymetry_averaged_down. It deliberately cannot detect "hires was ignored" -- that is
# Seamount_hires's job.
amr.max_level                     = 1
