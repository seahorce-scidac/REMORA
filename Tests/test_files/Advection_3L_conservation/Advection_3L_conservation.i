...
#
# Three levels, so a refluxed interface sits above a level that is itself substepped: level 2
# takes two steps per level-1 step, and level 1 two per level-0 step. The correction at the
# 1/2 interface therefore has to survive being accumulated across several steps of level 1
# before it is applied, which a two-level case cannot exercise.
#
