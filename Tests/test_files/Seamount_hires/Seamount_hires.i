...
# Bathymetry is evaluated analytically on level 1 and averaged down to level 0. Seamount's h is
# a Gaussian in physical x,y, so the mean of the 3x3 fine samples in a coarse cell is NOT the
# coarse midpoint sample: this lane must DISAGREE with the plain Seamount baseline, by ~16 m at
# the summit. That disagreement is the test -- see add_test_r_differ in Tests/CTestList.cmake.
# Keep ref_ratio at 3: cum_ref_ratios[1] then equals NGROW+1, the grow ring the analytic
# bathymetry hook writes.
amr.max_level                     = 1
