                                 ...
                                 int N, Real dt_lev)
{
    ...
    for (MFIter mfi(mf_cons_new, false); mfi.isValid(); ++mfi) {
        ...
        fennel_bridge_advance_c(
            ...
            do_bulk_flux ? 1 : 0,
            parms.river_don ? 1 : 0,
            parms.talk_nonconserv ? 1 : 0,
            // Enum ordinals rather than bools: these options are three- and
            // two-way choices, and the Fortran side compares them against the
            // CPP set it was compiled with.
            static_cast<int>(parms.pco2air_type),
            static_cast<int>(parms.co2_schmidt),
            static_cast<int>(parms.o2_schmidt),
            parms.BioIter,
            ...
            double(parms.SDeRRC),  double(parms.RDeRRN), double(parms.RDeRRC),
            double(parms.wPhy),    double(parms.wLDet),
