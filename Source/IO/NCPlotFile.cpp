...
void
REMORA::writeNCPlotFile(int lev, int which_subdomain, const std::string& dir,
...
{
     // For right now we assume single level
...
#endif
     {
         ...
         ncf.def_dim(xi_rho_name,  n_cells[0]);
         ncf.def_dim(xi_u_name,    n_cells[0]+1);
         ncf.def_dim(xi_v_name,    n_cells[0]);
         ncf.def_dim(xi_psi_name,  n_cells[0]+1);
         ncf.def_dim(xi_vert_name, n_cells[0]);

         ncf.def_dim(eta_rho_name,  n_cells[0]);
         ncf.def_dim(eta_u_name,    n_cells[0]);
         ncf.def_dim(eta_v_name,    n_cells[0]+1);
         ncf.def_dim(eta_psi_name,  n_cells[0]+1);
         ncf.def_dim(eta_vert_name, n_cells[0]);

         ncf.def_dim(s_r_name,    n_cells[2]);
         ncf.def_dim(s_w_name,    n_cells[2]+1);
         ...
         ncf.def_var(plot_var_names[0], NC_FLOAT, {nt_name, s_r_name, eta_rho_name, xi_rho_name});
         // }
#else
         // for (int i = 0; i < plot_var_names.size(); i++) {
...
*/
   ...
   std::vector<Real> vert_grid;//(box.length(2));
   ...
   for (amrex::MFIter fai(*plotMF[lev]); fai.isValid(); ++fai)
   {
       ...
       if (subdomain.contains(box)||true) {
          ...
          int ncomp_hack = 2; // only write temp and salt
          for (int k(0); k < ncomp_hack; ++k) {
              auto data = plotMF[lev]->get(fai).dataPtr(k);
              Print() << "NAME " << plot_var_names[k] << std::endl;
          ...
          {
          auto data = vec_x_r[lev]->get(fai).dataPtr();
          ...
          {
          auto data = vec_y_r[lev]->get(fai).dataPtr();
          ...
          {
          auto data = vec_z_r[lev]->get(fai).dataPtr();
          ...
          {
          auto data = vec_z_w[lev]->get(fai).dataPtr();
          ...
          {
          auto data = vec_s_r[lev]->get(fai).dataPtr();
          ...
          // x_grid.clear()   ;    x_grid.resize((unsigned long) box.length(0));
          // y_grid.clear()   ;    y_grid.resize((unsigned long) box.length(1));
          z_r_grid.clear() ;  z_r_grid.resize((unsigned long) box.length(2));
          z_w_grid.clear() ;  z_w_grid.resize((unsigned long) box.length(2));

          xi_grid.clear()  ;   xi_grid.resize((unsigned long) box.length(0));
          eta_grid.clear() ;  eta_grid.resize((unsigned long) box.length(1));
          vert_grid.clear(); vert_grid.resize((unsigned long) box.length(2));
              ...
              for(int k=(unsigned long) box.smallEnd(2); k<=box.bigEnd(2);k++)
          {
            ...
            const auto dx              = geomdata.CellSize();
               ...
               // x_grid[i-(unsigned long) box.smallEnd(0)] = prob_lo[0] + (i + 0.5) * dx[0];
              xi_grid[i-(unsigned long) box.smallEnd(0)] = i;

               // y_grid[j-(unsigned long) box.smallEnd(1)] = prob_lo[1] + (j + 0.5) * dx[1];
             eta_grid[j-(unsigned long) box.smallEnd(1)] = j;

             z_w_grid[k-(unsigned long) box.smallEnd(2)] = z_w(i,j,k);
            vert_grid[k-(unsigned long) box.smallEnd(2)] = z_r(i,j,k);
          }
