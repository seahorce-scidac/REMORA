    ...
    if (!is_periodic_in_x)
    {
        ...
        ParallelFor(bx_xlo, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) {
                ...
                if (bc_ptr[n].lo(0) == REMORABCType::foextrap) {
                    dest_arr(i,j,k,icomp+n) =  dest_arr(dom_lo.x-real_bcs[n],j,k,icomp+n);
            ...
            bx_xhi, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) {
                ...
                if (bc_ptr[n].hi(0) == REMORABCType::foextrap) {
                    dest_arr(i,j,k,icomp+n) =  dest_arr(dom_hi.x+real_bcs[n],j,k,icomp+n);
        ...
        ParallelFor(
            bx_ylo, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) {
                ...
                if (bc_ptr[n].lo(1) == REMORABCType::foextrap) {
                    dest_arr(i,j,k,icomp+n) =  dest_arr(i,dom_lo.y-real_bcs[n],k,icomp+n);
            ...
            bx_yhi, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) {
                ...
                if (bc_ptr[n].hi(1) == REMORABCType::foextrap) {
                    dest_arr(i,j,k,icomp+n) =  dest_arr(i,dom_hi.y+real_bcs[n],k,icomp+n);
