        ...
        ParallelFor(
            bx_xlo & dest_arr_box, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) {
                if (bc_ptr[n].lo(0) == REMORABCType::ext_dir) {
                    ...
                    Real grad_lo       = (calc_arr(dom_lo.x  ,j  ,k,icomp_calc+n) - calc_arr(dom_lo.x  ,j-1,k,icomp_calc+n)) * mskv(dom_lo.x,j  ,0);
                    Real grad_lo_jp1   = (calc_arr(dom_lo.x  ,j+1,k,icomp_calc+n) - calc_arr(dom_lo.x  ,j  ,k,icomp_calc+n)) * mskv(dom_lo.x,j+1,0);
        ...
        ParallelFor(
            bx_ylo & dest_arr_box, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) {
                if (bc_ptr[n].lo(1) == REMORABCType::ext_dir) {
                    ...
                    Real grad_lo       = (calc_arr(i  ,dom_lo.y,  k,icomp_calc+n) - calc_arr(i-1,dom_lo.y  ,k,icomp_calc+n)) * msku(i  ,dom_lo.y,0);
                    Real grad_lo_ip1   = (calc_arr(i+1,dom_lo.y  ,k,icomp_calc+n) - calc_arr(i  ,dom_lo.y  ,k,icomp_calc+n)) * msku(i+1,dom_lo.y,0);
            ...
            bx_yhi & dest_arr_box, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) {
                if (bc_ptr[n].hi(1) == REMORABCType::ext_dir) {
                    ...
                    Real grad_hi      = (calc_arr(i  ,dom_hi.y  ,k,icomp_calc+n) - calc_arr(i-1,dom_hi.y  ,k,icomp_calc+n)) * msku(i  ,dom_hi.y,0);
                    Real grad_hi_ip1  = (calc_arr(i+1,dom_hi.y  ,k,icomp_calc+n) - calc_arr(i  ,dom_hi.y  ,k,icomp_calc+n)) * msku(i+1,dom_hi.y,0);
