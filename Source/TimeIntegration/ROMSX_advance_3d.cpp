                   ...
                   const int ncomp, const int N, Real dt_lev)
{
    ...
    for ( MFIter mfi(mf_u, TilingIfNotGPU()); mfi.isValid(); ++mfi )
    {
        ...
        Array4<Real> const& DU_avg2_arr  = mf_DU_avg2->array(mfi);
        Array4<Real> const& DV_avg2_arr  = mf_DV_avg2->array(mfi);
	...
	Box ubx2 = surroundingNodes(ubx,0);
       ...
       update_massflux_3d(Box(Huon),1,0,u,Huon,Hz_arr,on_u,DU_avg1_arr,DU_avg2_arr,DC_arr,FC_arr,CF_arr,nnew);
       update_massflux_3d(Box(Hvom),0,1,v,Hvom,Hz_arr,om_v,DV_avg1_arr,DV_avg2_arr,DC_arr,FC_arr,CF_arr,nnew);
       */
       /////////////////////////// doesn't match boxes well, want the whole of Huon updated
