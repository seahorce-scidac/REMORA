                 ...
                 int nrhs, int nr)
{
    ...
    {
        Real cff1 = Real(0.5) * (vold(i,j,k,nrhs) + vold(i  ,j+1,k,nrhs));
        Real cff2 = Real(0.5) * (uold(i,j,k,nrhs) + uold(i+1,j  ,k,nrhs));
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k)
    {
        ru(i,j,k,nr) += Real(0.5) * (UFx(i,j,k) + UFx(i-1,j,k));
    });

    ParallelFor(ybx,
    [=] AMREX_GPU_DEVICE (int i, int j, int k)
    {
        rv(i,j,k,nr) -= Real(0.5) * (VFe(i,j,k) + VFe(i,j-1,k));
    });
}
