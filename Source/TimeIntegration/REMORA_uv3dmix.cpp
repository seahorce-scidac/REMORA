                 ...
                 const Real dt_lev)
{
    ...
    FArrayBox fab(grow(bx,IntVect(2,2,0)),ncomp,amrex::The_Async_Arena()); fab.template setVal<RunOn::Device>(zero);
    ...
    {
        const Real cff = half*Hz(i,j,k) * ( pm(i,j,0) / pn(i,j,0) *
    ...
    ParallelFor(growHi(xbx,1,1), [=] AMREX_GPU_DEVICE (int i, int j, int k)
    {
        ...
        const Real cff = mskp(i,j,0) * Real(0.125) *
    ...
    {
        for (int k=0; k<=N; k++) {
            const Real cff=dt_lev*Real(0.25)*(pm(i-1,j,0)+pm(i,j,0))*(pn(i-1,j,0)+pn(i,j,0));
            const Real cff1=half*(pn(i-1,j,0)+pn(i,j,0))*(UFx(i,j  ,k)-UFx(i-1,j,k));
            const Real cff2=half*(pm(i-1,j,0)+pm(i,j,0))*(UFe(i,j+1,k)-UFe(i  ,j,k));
    ...
    ParallelFor(growHi(ybx,0,1), [=] AMREX_GPU_DEVICE (int i, int j, int k)
    {
        ...
        const Real pnom_p = (pn(i-1,j-1,0)+pn(i-1,j,0)+pn(i,j-1,0)+pn(i,j,0)) /
                            (pm(i-1,j-1,0)+pm(i-1,j,0)+pm(i,j-1,0)+pm(i,j,0));
        const Real cff = mskp(i,j,0) * Real(0.125) * (Hz(i-1,j  ,k)+Hz(i,j ,k)+
    ...
    {
        for (int k=0; k<=N; k++) {
            const Real cff=dt_lev*Real(0.25)*(pm(i,j,0)+pm(i,j-1,0))*(pn(i,j,0)+pn(i,j-1,0));
            const Real cff1=half*(pn(i,j-1,0)+pn(i,j,0))*(VFx(i+1,j,k)-VFx(i,j  ,k));
            const Real cff2=half*(pm(i,j-1,0)+pm(i,j,0))*(VFe(i  ,j,k)-VFe(i,j-1,k));
            const Real cff3=cff*(cff1-cff2);
