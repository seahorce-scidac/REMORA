...
ROMSX simulation is run and output files with relevant simulation data are stored at
regular time intervals. Once the ROMSX simulation is complete, an AMR-Wind simulation
begins, using the ROMSX output files as time-dependent boundary conditions or forcing.
...
The output files generated by ROMSX are NetCDF files of a format compatible with the
...
We note that the solution variables in ROMSX are
...
The following parameters control the generation of 1D column output files in ROMSX. To
...
by setting `USE_NETCDF = TRUE` in the GNUMakefile for ROMSX.

+----------------------------+------------------+------------------+-----------------+
| Parameter                  | Definition       | Acceptable       | Default         |
|                            |                  | Values           |                 |
+============================+==================+==================+=================+
| **romsx.output_1d_column**   | whether to output| 0 or 1           | 0               |
|                            | for coupling     |                  |                 |
|                            | to AMR-Wind      |                  |                 |
+----------------------------+------------------+------------------+-----------------+
| **romsx.column_file_name**   | prefix for       | String           | “column_data.nc”|
|                            | output files     |                  |                 |
+----------------------------+------------------+------------------+-----------------+
| **romsx.column_interval**    | how often (by    | Integer          | -1              |
|                            | level-0 time     | :math:`> 0`      |                 |
|                            | steps) to output |                  |                 |
|                            | data files       |                  |                 |
+----------------------------+------------------+------------------+-----------------+
| **romsx.column_per**         | how often (by    | Real :math:`> 0` | -1.0            |
|                            | simulation time) |                  |                 |
|                            | to write output  |                  |                 |
|                            | data files       |                  |                 |
+----------------------------+------------------+------------------+-----------------+
| **romsx.column_loc_x**       | x-coordinate     | prob_lo(0) <= x  | 0.0             |
|                            | where vertical   | <= prob_hi(0)    |                 |
|                            | profile will be  |                  |                 |
|                            | extracted        |                  |                 |
+----------------------------+------------------+------------------+-----------------+
| **romsx.column_loc_y**       | y-coordinate     | prob_lo(1) <= y  | 0.0             |
...
*  You should specify either **romsx.output_int** or **romsx.output_per**, but not both.

2D File-based coupling
----------------------

For 2D file-based coupling to AMR-Wind, vertical planes are saved from an ROMSX simulation
...
These files are specifically meant to be read by AMR-Wind, which is also AMReX-based, or ROMSX itself.
...
.. code-block:: none
  ...
  romsx.output_bndry_planes = 1
  romsx.bndry_output_planes_interval = 2
  romsx.bndry_output_start_time = 0.0
  romsx.bndry_output_planes_file = "BndryFiles"
  romsx.bndry_output_var_names = temperature velocity density
  romsx.bndry_output_box_lo = 256. 256.
  romsx.bndry_output_box_hi = 768. 768.
...
We also have the functionality in ROMSX to read in these types of files;
for this one would add the following (or similar) line to the inputs file:

.. code-block:: none

  romsx.input_bndry_planes = 1
  romsx.bndry_file = "BndryFiles"
  romsx.bndry_input_var_names = density temperature velocity

When run with these inputs, ROMSX will read in the time sequence of files contained in the folder :cpp:`BndryFiles`,
...
domain specified by :cpp:`bndry_output_box_lo` and :cpp:`bndry_output_box_hi` when the files were written.  If not, ROMSX will
