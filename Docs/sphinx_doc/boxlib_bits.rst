...
Function Listing for ROMSX
--------------------------

When built, the full doxygen documentation for ROMSX can be found
`here <../../../doxygen_output/html/index.html>`_.



AMReX functions useful for ROMSX development
-------------------------------------------

ROMSX is built on AMReX (available at `https://github.com/AMReX-Codes/amrex <https://github.com/AMReX-Codes/amrex>`_),
...
operations. Below is a quick reference list with links to many of the AMReX tools used to build up ROMSX. The full AMReX documentation can be found `here <https://amrex-codes.github.io/AMReXUsersGuide.pdf>`_.
...
Contributing to ROMSX
---------------------

Development Model
~~~~~~~~~~~~~~~~~

To add a new feature to ROMSX, the procedure is:
...
3. Push feature branch to ROMSX repository ::
...
4.  Submit a merge request through git@github.com:AMReX-Combustion/ROMSX.git - be sure you are requesting to merge your branch to the development branch.
...
It is possibly---and desirable---to create the regression testing framework locally and ensure that all tests pass successfully. This is also a good way to ensure all is properly installed on a new machine to be used for ROMSX calculations. The initial setup is somewhat tedious but is worth the effort. What needs to be done is: (1) make a scratch area on the local machine where you manually run the regression tests.  As part of the process, a set of "gold" solutions will be generated using code from the current versions of ROMSX and amrex.  Regression tests afterwards will compare to those solutions and indicate binary compatibility. (2) Clone the required repositories into this area and set required environment variables that point to where everything is. (3) Run the tests to generate the "gold" benchmark data.
...
2. Clone repositories (amrex, ROMSX, regression_testing (AMReX's driver scripts) and ROMSXRegressionTesting (ROMSX-specific stuff)) ::

     git clone git@github.com:romsx.model/ROMSXRegressionTesting.git
     cd ROMSXRegressionTesting; git checkout development
     mkdir -p TestData/ROMSX  # this is where the test results will be written
     mkdir Repositories   # this is where the src code to be tested is put
     cd Repositories
     export ROMSX_HOME=`pwd`/ROMSX; git clone git@github.com:AMReX-Combustion/ROMSX.git $ROMSX_HOME
     cd $ROMSX_HOME; git checkout development; cd ..
...
3. Run the script to execute the tests to generate benchmarks. After it finishes building and running (12 as of March 2019) tests, it will archive the pltfiles that result from each into a folder in the TestData/ROMSX folder in the ROMSXRegressionTesting folder.  Once the benchmarks exist, any changes to the repositories in the ROMSXRegressionTesting/Repositories can be tested to diff clean against these benchmarks by running the script (again from within the ROMSXRegressionTesting folder) ::
